...
export default class SanitizedReporter implements Reporter {
  ...
  onEnd(result: FullResult): void {
    ...
    const receipt = { schemaVersion: 2, status: statuses.has(result.status) ? result.status : 'unknown', durationMs: Number.isFinite(result.duration) ? Math.max(0,Math.round(result.duration)) : 0, counts, tests: this.#results };
    ...
    process.stdout.write(`Browser QA: ${counts.passed} passed, ${counts.failed} failed, ${counts.timedOut} timed out, ${counts.skipped} skipped, ${counts.interrupted} interrupted; ${receipt.durationMs}ms.\n`);
