...
export function classify(paths) {
  const result = { app: false, docs: false, site: false, packaging: false };
  for (const path of paths) {
    if (path.startsWith('site/')) result.site = true;
    else if (/^(README\.md|docs\/.*\.md|[A-Z_-]+\.md)$/.test(path)) result.docs = true;
    else result.app = true; // Unknown files, fixtures, dependencies and CI changes fail closed.
    if (path === 'README.md' || path.startsWith('docs/public-beta/')) result.packaging = true;
  }
  if (!paths.length) result.app = true;
  return result;
}
