...
The result contains an editor ID, session ID, base revision, and layer IDs. It does not expose the workspace path, source SVG, registry records, or a token. Use those exact values in a versioned proposal. For example, a harmless review selection is:
...
```json
{
  ...
  "document": { "sessionId": "<sessionId>", "baseRevision": 0 },
  "operations": [{ "type": "selectFocus", "operationId": "focus-title", "targets": [{ "sessionKey": "<layerId>" }] }]
}
```
...
The command refuses an ambiguous or stale editor context, malformed proposal, unsafe SVG, and any proposal containing an unexpected field such as `sourcePath`. It waits for a person to review the proposal. The person must explicitly choose **Accept all** in the editor; accepting is not completion until the editor reports a saved `iterations/...svg` receipt.
