# Owner-run stable release

`Publish release` is deliberately a manual, inert-until-owner-configured release
workflow. It publishes a single new public stable `lineage-logo` version only after
...
be `public`, `publishConfig.tag` to be `latest`, and no `publishConfig.registry`.
The workflow checks those observable repository, workflow, package, and publish
values immediately before publishing. The OIDC connection was proven by the
0.1.0-beta.4 publication. The stable `latest` path is still **unproven** until
an explicitly authorized hosted run succeeds.
Do not configure a broader, staged, or other npm action as a substitute.

A successful hosted run is an owner-only action. This repository workflow alone
is not evidence that trusted publishing or provenance is operational for a new
version. Do not dispatch it, alter the environment, authenticate, or publish
until the owner explicitly authorizes the live validation.
...
Enter a new exact stable version matching `package.json`, in the form `0.1.0`.
Every major, minor, and patch numeric identifier must be either `0` or start
with `1`–`9`; leading-zero versions such as `00.1.0`, `0.01.0`, and `0.1.00`
are refused. Tags such as `beta` or `latest`, version ranges, prefixes, and
prerelease versions are also refused. The workflow accepts version absence only when
...
The only publish command is `npm publish <verified-tarball> --tag latest --provenance`, which assigns
the `latest` dist-tag to the newly published version. After the dist-tag snapshot,
...
tarball. The workflow does not invoke a separate `npm dist-tag add` or
`npm dist-tag rm` repair. It requires `latest` to resolve to the exact new
version after publication. It captures a machine-readable dist-tag map
immediately before publish and compares it after publish: only `latest` may
change, and it must point to the requested exact version. Every other tag,
including `beta`, must remain byte-for-byte unchanged; addition, removal, or
...
corrected in a new patch version after the prepublish gates pass. Observe the
