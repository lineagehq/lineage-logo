# Public beta walkthrough invitation

This is final reusable invitation copy. It is not authorization to select or
contact anyone. Fill every placeholder and obtain the owner approvals below
before sending it through any channel.

## Delivery placeholders

- Recipient: `<RECIPIENT>`
- Delivery channel: `<OWNER_APPROVED_CHANNEL>`
- Pseudonymous participant slot: `<P-NNN>`
- Walkthrough ID: `<W-6_TO_32_UPPERCASE_LETTERS_OR_DIGITS>`
- Receipt return handling: `<OWNER_APPROVED_PRIVACY_SAFE_RETURN_INSTRUCTIONS>`

## Invitation copy

**Subject:** Invitation: unaided Lineage Logo public-beta walkthrough

Hello `<RECIPIENT>`,

We are inviting you to try one independent, unaided walkthrough of Lineage
Logo's published public beta. This is an early beta, not a claim that the
workflow has already been validated by external users. The current independent
walkthrough count is **0/3**.

Please participate only if you can use:

- Node.js 22 or newer;
- macOS or Linux; and
- Chromium.

Use only the published npm package selected by `lineage-logo@beta`. Do not use a
source checkout, local tarball, linked package, prerelease supplied privately,
or another install source. Follow these shipped documents in order:
...
save, and clean reopen. Record the exact version printed after installation.
The maintainer cannot coach, hint, take control, answer step-by-step questions,
or repair your environment during the attempt. Use only the shipped instructions
and normal product help. If you become blocked, apply only the protocol's bounded
recovery rule and record the controlled result.

Your receipt uses the pseudonymous slot `<P-NNN>` and walkthrough ID
`<W-6_TO_32_UPPERCASE_LETTERS_OR_DIGITS>`. It records controlled statuses,
durations, friction codes, environment fields, and the exact installed version;
it must not include your name, email, contact details, or any reidentification
...
Before delivery, the maintainer assigns one unused pseudonymous participant slot
