...
| --- | --- | --- |
| Create/import, structural proposal, exact revision feedback and preserving follow-up | `tests/e2e/guided-creation-installed.spec.ts`; `evidence/g4-installed.json` | Installed public CLI and UI passed on macOS; final Linux confirmation pending. |
...
| Browser engines, corpus, zoom, accessibility and current LTS | `integration/d2.md`, `evidence/d2.json`, `evidence/d2-packages/` | PR38 CI passed at62b20a5; test-only synchronization fix requires fresh CI. Real assistive-technology observation remains unverified. |
